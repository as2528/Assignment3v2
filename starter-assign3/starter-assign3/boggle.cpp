...
bool recursiveSolver(Grid<char>& board, Lexicon& lex,string word,GridLocation currentPosition, Grid<bool>locations, Set<GridLocation>allNeighbors, Set<string>wordList){
   // int points =0;
 //   int rows=0;
   // int columns=0;
    haveVisited(locations,currentPosition);
    word+=board.get(currentPosition);
    ...
    if(allNeighbors.isEmpty()){
        ...
        recursiveSolver(board,lex,word, currentPosition,locations, generateValidMoves(board,currentPosition),wordList);
        return false;
    }
...
int scoreBoard(Grid<char>& board, Lexicon& lex) {//This function will backtrace until it finds the possible words on boggle board
  /* TODO: Implement this function.
   ...
   /* int result =0;
    ...
    for(int i=0;i<=3;i++){
        for(int j=0;j<=3;j++){
            GridLocation startPosition={i,j};
            string currentWord="";
            (recursiveSolver(board,lex,currentWord, startPosition,locations, generateValidMoves(board,startPosition),wordList));
//Grid<char>& board, Lexicon& lex,string word,GridLocation currentPosition, Grid<bool>locations, Set<GridLocation>allNeighbors,Set<string>previousWords
